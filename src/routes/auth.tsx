...
import { Loader2, MessagesSquare, ShieldCheck, Sparkles } from "lucide-react";
...
const HIGHLIGHTS = [
  { icon: Sparkles, text: "AI ajanları saniyeler içinde arıza teşhisi çıkarır" },
  { icon: MessagesSquare, text: "Binlerce usta ve sürücüden topluluk doğrulaması" },
  { icon: ShieldCheck, text: "Kişisel garaj ile araca özel kronik sorun takibi" },
];
  ...
  return (
    <div className="grid min-h-screen lg:grid-cols-2">
      <div className="relative hidden flex-col justify-between overflow-hidden bg-brand-gradient px-12 py-12 text-white lg:flex">
        <div className="node-orbit-bg pointer-events-none absolute inset-0 opacity-70" />
        <Link to="/" className="relative flex items-center gap-2.5 text-lg font-bold">
          <span className="flex h-9 w-9 items-center justify-center overflow-hidden rounded-xl bg-white/10">
            <img src={logoAsset.url} alt="" className="h-full w-full object-cover" />
          </span>
          <span className="font-display">AutoSocial</span>
        </Link>
        ...
        <p className="relative text-xs text-white/40">© {new Date().getFullYear()} AutoSocial</p>
      </div>

      <div className="node-orbit-bg flex items-center justify-center bg-background px-4 py-10">
        <div className="w-full max-w-md">
          <div className="mb-8 text-center lg:hidden">
            <Link to="/" className="inline-flex items-center gap-2.5">
              <span className="flex h-9 w-9 items-center justify-center overflow-hidden rounded-xl bg-brand-gradient">
                <img src={logoAsset.url} alt="" className="h-full w-full object-cover" />
              </span>
              <span className="font-display text-xl font-bold">AutoSocial</span>
            </Link>
          </div>

          <div className="rounded-2xl border border-border bg-card p-6 shadow-xl shadow-black/[0.03]">
